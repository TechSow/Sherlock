...
public class MateriaDAO extends BaseDAO implements IMateriaRepository{
	...
	public ArrayList<Materia> getMateriasFromCursoId(int id_curso) throws Exception {
		...
		stmt = conn.prepareStatement("SELECT ID_MATERIA, NOME_MATERIA, EMENTA_MATERIA FROM TS_T_CURSO_MATERIA INNER JOIN TS_T_CURSO ON TS_T_CURSO_MATERIA.FK_ID_CURSO = TS_T_CURSO.ID_CURSO INNER JOIN TS_T_MATERIA ON TS_T_MATERIA.ID_MATERIA = TS_T_CURSO_MATERIA.FK_ID_MATERIA WHERE FK_ID_CURSO = ?");
		...
		while(rs.next()) {
			...
			materias.add(new Materia(id,nome,ementa));
